...
def send_requests(number, headers):
    urls = [
    ...
    for url in urls:
        try:
            ...
            print(f"Запрос к {url} успешен. Код ответа: {response.status_code}")
...
def spamr():
    while True:
        number = int(input("Введите номер телефона для спама: "))
        povtor = int(input("Количество повторов: "))
        count = 0

        try:
            for _ in range(povtor):
                user = fake_useragent.UserAgent().random
                headers = {'user-agent': user}
                
                print(f"Юзерагент: {headers}")
